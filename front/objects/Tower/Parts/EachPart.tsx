...
const EachPart: FC<Props> = ({ draws, currentPartIndex }) => {
    var totalHeight = INIT_CENTER.y
    draws.slice(currentPartIndex, currentPartIndex + 1).forEach((e) => {
    ...
    GUIDE_TEXT_SIZE = 500
    //TOTAL GUIDE
    TOTAL_GUIDE_MARGIN = draws[0].bottom / 2 + GUIDE_MARGIN + GUIDE_TEXT_SIZE * 1
    TOTAL_GUIDE_LINE_WIDTH = LINE_WIDTH / 2
    TOTAL_GUIDE_TEXT_SIZE = 500
    ...
    return (
        <svg viewBox={`${viewCenterMargin} ${-viewHeight} ${viewWidth} ${viewHeight}`} fill="#fff">
            <Square
                key={`each-section-${currentPartIndex}`}
                center={INIT_CENTER}
                draw={draws[currentPartIndex]}
                lineColor={LINE_COLOR_ACTIVE}
                lineWidth={LINE_WIDTH_ACTIVE}
                guideEnable={false}
            ...
            {GUIDE_ENABLE && (
                <>
                    <g>
                        ...
                        <Guide
                            pointStart={{
                                x: -draws[currentPartIndex].top / 2,
                        ...
                        <Guide
                            pointStart={{
                                x: -draws[currentPartIndex].bottom / 2,
                                y: INIT_CENTER.y,
                            }}
                            pointEnd={{ x: draws[currentPartIndex].bottom / 2, y: INIT_CENTER.y }}
                            guideMargin={GUIDE_MARGIN}
